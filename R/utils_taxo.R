...
get_species_photo <- function(name){
  ...
  if(any(names(resp)=='query')){
    for (i in 1:length(resp$query$pages)){
      if(grepl("File:Commons-logo",resp$query$pages[i][[1]]$title, fixed = TRUE)==FALSE & grepl("File:Open Access",resp$query$pages[i][[1]]$title, fixed = TRUE)==FALSE & grepl("Disambig",resp$query$pages[i][[1]]$title, fixed = TRUE)==FALSE & grepl("Closed_Access_logo_alternative",resp$query$pages[i][[1]]$imageinfo[[1]]$thumburl, fixed = TRUE)==FALSE) {
